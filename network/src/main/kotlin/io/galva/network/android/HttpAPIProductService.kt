...
class HttpAPIProductService(
    private val baseURL: String,
    private val httpClient: HttpClient,
    private val logger: Logger,
    private val json: Json = JsonUtils.defaultJson,
) : ProductService {
    override suspend fun getProducts(request: ListProductRequest): ServiceResult<ListProductResponse> {
        return try {
            val request =
                RequestBuilder.get(Endpoint.Products.toUrl(baseURL))
                    .query("limit", request.limit)
                    .query("page", request.page)
                    .query("status", request.status)
                    .build()
            logger.info {
                "getProducts request: ${request.url}, headers: ${request.headers}"
            }
            httpClient.execute(request).toServiceResult { responseJson ->
                json.decodeFromString<ListProductResponse>(responseJson)
            }
        } catch (e: NetworkError) {
            logger.error {
                "getProducts failed with NetworkError: ${e.message}, cause: ${e.cause?.message}"
            }
            ServiceResult.Error(e)
        } catch (e: Exception) {
            logger.error {
                "getProducts failed with Exception: ${e.message}, cause: ${e.cause?.message}"
            }
